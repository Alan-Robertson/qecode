						...
						void* decoder_data)
{
	...
	// Vectorise the Krauss operators to get the channel
	channel = dmatrix_vectorise(&sum_physical_operator);
...
MatrixXcd channel_logical_krauss(const sym* code, 
						const sym* logicals, 
						double (*error_model)(const sym*, void*), 
						void* model_data, 
						sym* (*decoder)(const sym*, void*),
...
{
	const unsigned krauss_size = (1 << (logicals->length/2));
	MatrixXcd logical_krauss_operator = dmatrix_zeros(krauss_size, krauss_size);
	...
	while (sym_iter_next(physical_error)) {
		...
		// Add this particular matrix with the appropriate weighting to the sum
		logical_krauss_operator += sqrt(error_prob) * logical_operator;
	...
	return logical_krauss_operator;
}

MatrixXcd channel_physical_krauss(const sym* code, 
						const sym* logicals, 
						double (*error_model)(const sym*, void*), 
						void* model_data, 
						sym* (*decoder)(const sym*, void*),
...
{
	const unsigned krauss_size = (1 << (code->length/2));	
	MatrixXcd physical_krauss_operator = dmatrix_zeros(krauss_size, krauss_size);
	...
	while (sym_iter_next(physical_error)) {
		...
		if (0 != error_prob)
		{
			...
			// Add this particular matrix with the appropriate weighting to the sum
			physical_krauss_operator += sqrt(error_prob) * physical_operator; 
	...
	return physical_krauss_operator;
}
