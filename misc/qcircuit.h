...
char* str_increase(char* original, char* added);


void qcircuit_print(circuit* c)
{
	char* header = "\\Qcircuit @C=1em @R=.7em {";
	char* space = "&";
	char* gate = "\\gate{";
	char* tail = "}";
	char* wire = "\\qw";
	...
	while (NULL != ce)
	{
		...
		for (uint32_t i = 0; i < c->n_qubits; i++)
		{
			...
			for (uint32_t j = 0; j < ce->gate->n_qubits && no_operation; j++)
			{
